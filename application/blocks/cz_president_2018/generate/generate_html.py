...
# load settings.json
with open("../settings.yaml") as fin:
    settings = yaml.load(fin)
with open(settings['block_reverse_path_dir'] + "../server_settings.json") as fin:
    server_settings = json.load(fin)

templateLoader = jinja2.FileSystemLoader( searchpath=["./templates", server_settings['app_dir'] + "templates"] )
templateEnv = jinja2.Environment( loader=templateLoader )

# app = Flask(__name__)

with open("../estimate/candidates_estimated.json") as fin:
    candidates = json.load(fin)

# date
stats = os.stat("../estimate/candidates_estimated.json")
...
for lang in langs:
    ...
    with open(server_settings['app_dir'] + "languages/texts." + lang + ".yaml") as fin:
        texts = yaml.load(fin)
    with open("../texts." + lang + ".yaml") as fin:
    ...
    html = core
    with open("../block." + lang + ".html","w") as fout:
        fout.write(html)
    ...
    html = core
    # print(html[:100])
    with open("../pictures/picture." + lang + ".html","w") as fout:
        fout.write(html)
    ...
    with open("../pictures/thumbnail." + lang + ".html","w") as fout:
        fout.write(html)
    # generate png
    cmd = 'xvfb-run --server-args="-screen 0, 1200x800x24" --auto-servernum /usr/bin/cutycapt --min-width=780 --min-height=528 --url=' + server_settings['app_url'] + settings['block_path_url'] + 'pictures/thumbnail.' + lang + '.html --out=' + server_settings['app_dir'] + settings['block_path_url'] + 'pictures/bigthumbnail.' + lang + '.' + str(stats.st_mtime) + '.png'
    call_params = shlex.split(cmd)
    proc = subprocess.Popen(call_params)
    proc.communicate()
    ...
    with open("../block." + lang + ".yaml", "w") as fout:
